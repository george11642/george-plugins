...
## D3 v7 Patterns & Observable Plot (2026)

### D3 v7 Key Changes
- **ESM-only**: `import * as d3 from "d3"` or import individual modules
- **`.join()` replaces enter/update/exit** for most use cases (already shown above)
- **Typed events**: `d3.pointer(event)` replaces deprecated `d3.mouse()`
- **No more `d3.event`**: event is passed as first argument to handlers

### Observable Plot (High-Level Alternative)

For standard charts where D3 is overkill, consider [Observable Plot](https://observablehq.com/plot/) -- a concise API built on D3:
...
**When to use Observable Plot vs D3**: Plot for exploratory analysis, standard charts, quick prototypes. D3 for custom layouts, physics simulations, bespoke interactivity, pixel-level control.
