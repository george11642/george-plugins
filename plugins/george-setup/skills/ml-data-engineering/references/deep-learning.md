...
## PyTorch 2.x Compilation & Distributed Training (2026)

### torch.compile

`torch.compile` is the primary optimization API in PyTorch 2.x+. It traces the model graph and fuses operations via TorchInductor.
...
# Basic compilation — wrap after model is on device
compiled_model = torch.compile(model)

# Production options
compiled_model = torch.compile(model, mode="reduce-overhead")  # Best for small batches
compiled_model = torch.compile(model, mode="max-autotune")      # Best throughput, slow compile
compiled_model = torch.compile(model, fullgraph=True)           # Error if graph break detected
compiled_model = torch.compile(model, dynamic=True)              # Handle varying input shapes

# Regional compilation — compile repeated blocks instead of full model
for layer in model.transformer_layers:
    layer = torch.compile(layer)
```

**Best practices (2026)**:
- Always establish a correct eager baseline first
- Minimize graph breaks — avoid Python control flow in forward()
- Use `fullgraph=True` during development to catch graph breaks
- Use `dynamic=True` if input shapes vary (avoids recompilation)
- Account for warmup: first forward pass triggers JIT compilation
- Regional compilation for large models: compile one layer, reuse across all
...
# FSDP2 — simpler API than FSDP1
model = MyModel()
fully_shard(model)  # Shards parameters across GPUs
...
### Key PyTorch 2.x modules
- `torch.distributed.fsdp.fully_shard` — FSDP2 simplified API
