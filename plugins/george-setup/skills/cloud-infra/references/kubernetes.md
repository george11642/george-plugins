...
## Native Sidecar Containers (K8s 1.29+ beta, stable 1.32+)

Native sidecar containers are init containers with `restartPolicy: Always`. They start before main containers and run alongside them, replacing the workaround of putting sidecars in the `containers` array.
...
Use cases: log shippers, service mesh proxies, monitoring agents. Verify feature gate: `kubectl get --raw /api/v1/nodes/<node>/proxy/metrics | grep SidecarContainers`.
...
6. **Sidecar ordering** -- native sidecar containers (init with `restartPolicy: Always`) start in order and must be healthy before main containers start
