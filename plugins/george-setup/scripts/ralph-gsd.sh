...
declare -A MILESTONE_PHASE_SET # set of phases defined in the current ROADMAP (for cross-milestone dep resolution)
PARALLEL_LOG_DIR=""
...
get_phase_status() {
    ...
    # Check for UAT.md (verify-work skill sometimes produces *-UAT.md instead of *VERIFICATION.md)
    local uat_file=""
    if [[ -f "$phase_path/UAT.md" ]]; then
        uat_file="$phase_path/UAT.md"
    else
        uat_file=$(find "$phase_path" -maxdepth 1 -name "*UAT.md" 2>/dev/null | head -1)
    fi

    if [[ -n "$uat_file" && -f "$uat_file" ]]; then
        if grep -qi "VERIFIED\|PASSED\|SUCCESS\|ALL.*COMPLETE" "$uat_file" 2>/dev/null; then
            echo "verified"
            return
        else
            # UAT file exists but indicates gaps
            echo "gaps"
            return
        fi
    fi
    ...
    # Check for CONTEXT.md or DISCUSSION.md (discuss complete) - bare and numbered variants
    if [[ -f "$phase_path/CONTEXT.md" ]] || [[ -f "$phase_path/DISCUSSION.md" ]]; then
        echo "context_captured"
        return
    fi
    if compgen -G "$phase_path/*CONTEXT.md" > /dev/null 2>&1; then
        echo "context_captured"
        return
    fi
    if compgen -G "$phase_path/*DISCUSSION.md" > /dev/null 2>&1; then
        echo "context_captured"
        return
    fi
...
parse_all_dependencies() {
    ...
    # Build MILESTONE_PHASE_SET: the set of phase numbers defined in this ROADMAP.
    # Dependencies on phases NOT in this set are assumed to be from prior completed
    # milestones and are treated as auto-satisfied in are_dependencies_met().
    local all_current_phases
    all_current_phases=$(grep -oE '^#{2,}[[:space:]]*Phase[[:space:]]+[0-9]+' "$roadmap" | grep -oE '[0-9]+' | sort -u -n)
    for p in $all_current_phases; do
        MILESTONE_PHASE_SET["$p"]=1
    done
...
are_dependencies_met() {
    ...
    for dep in $deps; do
        # If the dependency phase is not in the current ROADMAP's phase list,
        # it belongs to a prior completed milestone — treat as auto-satisfied.
        if [[ -z "${MILESTONE_PHASE_SET[$dep]:-}" ]]; then
            continue
        fi
